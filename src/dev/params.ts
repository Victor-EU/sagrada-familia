...
export function buildPanel(ctx: ParamContext): Pane {
  ...
  shell.addBinding(ctx.plan.shell, 'skylightClear', { min: 0, max: 5, step: 0.1, label: 'skylight clear m' })
  shell.addBinding(ctx.plan.shell, 'curb', { min: 0, max: 6, step: 0.1, label: 'lantern curb m' })
  shell.on('change', () => ctx.rebuild())

  const towers = pane.addFolder({ title: 'Towers' })
  towers.addBinding(ctx.plan.towers, 'show', { label: 'towers' })
  towers.addBinding(ctx.plan.towers, 'apostles', { label: 'the twelve' })
  towers.addBinding(ctx.plan.towers, 'central', { label: 'Jesus, Mary, the four' })
  towers.addBinding(ctx.plan.towers, 'taper', { min: 0.1, max: 0.9, step: 0.01, label: 'taper' })
  towers.addBinding(ctx.plan.towers, 'star', { min: 0, max: 1, step: 0.01, label: 'star in section' })
  towers.addBinding(ctx.plan.towers, 'twistDeg', { min: -90, max: 90, step: 1, label: 'twist °' })
  towers.addBinding(ctx.plan.towers, 'pitch', { min: 0.8, max: 6, step: 0.05, label: 'row pitch m' })
  towers.addBinding(ctx.plan.towers, 'tall', { min: 0.1, max: 0.9, step: 0.02, label: 'aperture tall' })
  towers.addBinding(ctx.plan.towers, 'wide', { min: 0.1, max: 0.9, step: 0.02, label: 'aperture wide' })
  towers.addBinding(ctx.plan.towers, 'lintel', { min: 0, max: 2, step: 0.05, label: 'hood m' })
  towers.addBinding(ctx.plan.towers, 'reveal', { min: 0, max: 2, step: 0.05, label: 'reveal m' })
  towers.addBinding(ctx.plan.towers, 'pinnacle', { min: 0.05, max: 0.4, step: 0.01, label: 'pinnacle share' })
  towers.on('change', () => ctx.rebuild())
  ...
  film.addBinding(ctx.render.look, 'grain', { min: 0, max: 0.08, step: 0.002 })
  film.addBinding(ctx.render.look, 'vignette', { min: 0, max: 0.6, step: 0.01 })
  film.addBinding(ctx.render.look, 'halation', { min: 0, max: 1.5, step: 0.01 })
